...
export const handelChartData = (data: any, type: string) => {
  const chartData: any[] = [];
  let lineHourlyData: any = {};

  data.forEach((chart: any) => {
    const { date_and_time, garages } = chart;
    if (type === "lastday") {
      // if (type === "today" || type === "lastday") {
      lineHourlyData = { time: `${moment(date_and_time).format("hh:mm")}pm` };
    } else {
      lineHourlyData = { time: moment(date_and_time).format("DD/MM") };
    }

    for (const garageName in garages) {
      // const index = data.findIndex((d) => d.name === garageName);
      lineHourlyData[garageName] = garages[garageName].spaces_left;
    }

    chartData.push(lineHourlyData);
